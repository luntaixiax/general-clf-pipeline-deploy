...
from src.services.eda.apis.apis import RemoteDataManagerForRegistry

def get_existing_data_ids_from_data_registry() -> List[str]:
    return RemoteDataManagerForRegistry.get_existing_data_ids_from_data_registry()

def get_existing_eda_model_ids() -> List[str]:
    return RemoteDataManagerForRegistry.get_existing_eda_model_ids()

def get_existing_eda_data_source_config(eda_model_id: str) -> dict:
    return RemoteDataManagerForRegistry.get_existing_eda_data_source_config(eda_model_id)

def get_existing_eda_model_profile(eda_model_id: str) -> list:
    return RemoteDataManagerForRegistry.get_existing_eda_model_profile(eda_model_id)
...
def delete_eda_model(eda_model_id: str):
    RemoteDataManagerForRegistry.delete_eda_model(eda_model_id)
...
if model_source_ == 'Start New EDA Modeling':
    ...
    data_source_ = st.radio(
        ...
        options = ['Data Registry', 'Query to Data Warehouse'],
        index = 0,
        horizontal = True
    )
    ...
    if data_source_ == 'Data Registry':
        ...
        query_ = st.text_area(
            label = 'SQL Query',
            placeholder = 'SELECT * FROM FEATURE.FEATURES'
        )
